...
class Beam:
    ...
    def advance(self, word_logprob):
        ...
        if len(self.prev_ks) > 0:
            beam_lk = word_logprob + self.scores.unsqueeze(1).expand_as(word_logprob) # beam_width , 98 
        else:
            # in initial case,
            beam_lk = word_logprob[0]


        #print(self.get_current_state().shape)
        #print(beam_lk.shape)

        if len(self.next_ys) == 1:
            pass
        else:
            self.scores=beam_lk[:,self.EOS]
            hyps = [self.get_hypothesis(k) for k in range(self.size)]
            #hyps = [[self.BOS] + h for h in hyps]
            hyps = [h for h in hyps]
            for i in range(self.size):
                if len(hyps[i])>=config.min_len:
                    self.nodes.put((-beam_lk[i][self.EOS].item(),hyps[i]))


        if len(self.next_ys)!=1:
            beam_lk[:,self.EOS]=torch.full((self.size,),float('-inf'))
...
def beam_search(model, src, src_mask, max_len, pad, bos, eos, beam_size, device, datastore, retriever, combiner):
    ...
    def beam_decode_step(
        ...
        def predict_word(dec_seq, enc_output, n_active_inst, n_bm):
            #print(enc_output[0]==enc_output[1])
            ...
            if config.use_datastore:
                word_logprob = model.generator(out[:, -1]) # batch_size*beam_width , 98
                word_prob=torch.exp(word_logprob)
                #word_logprob = word_logprob.view(n_active_inst, n_bm, -1) # batch_size , beam_width ,98
                #print(word_logprob.shape)
                #exit()
                #print(torch.exp(word_logprob))
                ...
                word_prob=word_prob*config.lambda_+knn_prob.squeeze(1)*(1-config.lambda_)
                word_logprob=torch.log(word_prob)
                word_logprob = word_logprob.view(n_active_inst, n_bm, -1) # batch_size , beam_width ,98
    ...
    with torch.no_grad():
        # -- Encode
        src_enc = model.encode(src, src_mask) # batrch_size , seq_len , 512
        #print(src_enc.shape)
    ...
    ans=[]
    for Be in inst_dec_beams:
        pq=Be.nodes
        #cnt=0
        res=[]
        while not pq.empty(): #and cnt<config.TopK:
            #cnt+=1
            x=pq.get()[1]
            res.append(x)
        ans+=[res]
