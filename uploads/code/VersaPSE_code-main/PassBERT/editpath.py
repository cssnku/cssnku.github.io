...
def inplace_edit(pwd1, pwd2, pseudo=False):
    ...
    if not pseudo:
        ...
        if is_arrive:
            ...
            return inplace_edit(pwd2, pwd2, pseudo=True)
...
def main():
    ...
    cli.add_argument("-c","--data",dest="csv",default="../dataset/train/pseudo_train_data-popular.txt",help="csv file of name and passwords pair")
    cli.add_argument("-o","--output",default="./dataset/inplace_edits_train_popular.txt",dest="output",help="Output file")
    cli.add_argument("--human",dest="human",action="store_true",default=False)
    cli.add_argument("--pseudo", type=bool, default=True, help="Use pseudo inplace edits")
    cli.add_argument("--src_pos", type=int, default=1, help='Position of the source text in each line.')
    cli.add_argument("--trg_pos", type=int, default=2, help='Position of the target text in each line.')
    args = cli.parse_args()
    ...
    for i,line in enumerate(f):
        lis=line.strip('\n').split('\t')
        src=lis[args.src_pos]
        tar=lis[args.trg_pos]
        # src=lis[0]
        # tar=lis[1]
        if len(src) > 30 or len(tar) > 30:
            continue
        res = inplace_edit(src, tar, pseudo=args.pseudo)
        stats.add(res)
        if stats.filter(src, res):
            ...
            res=inplace_edit(tar, tar, pseudo=True)
            if args.human:
                output.write(f"{src}\t{tar}\t{res[2]}\n")
            else:
                output.write(f"{src}\t{tar}\t{encode_inplace_edit(mapper, res[2])}\n")
