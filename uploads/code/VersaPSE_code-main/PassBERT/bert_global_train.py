...
if __name__ == "__main__":
    ...
    parser.add_argument('--train_file', type=str, default="./dataset/inplace_edits_train_popular.txt")
    parser.add_argument('--load_ckpt', type=str, default="./bert_local_last.pth")
    parser.add_argument('--save_path', type=str, default="./bert_local_to_global.pth")
    parser.add_argument('--epochs', type=int, default=5)

    args = parser.parse_args()

    # Global training: start from a local checkpoint, freeze most layers
    # (only the lower BERT encoder layers remain trainable) and fine-tune
    # on a large global dataset (rockyou / tianya).
    model = PassBERT()
    model.to('cuda')
    model.freeze_layers()
